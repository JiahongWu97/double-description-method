...
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r_g_set.exe rank_only_NC2.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/lex_set.exe rank_only_NC2.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/lex_rank.exe rank_only_NC2.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r_g_rank.exe rank_only_NC2.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/tell.exe tell_me_array_dim.c
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r.exe rank_only_NC2.c -pthread


int ineq[m][n], ineq_order[m], (*er)[n], ii, *birth_index;
_Bool (*active_ineq)[m];
struct er_new_info *head, *prev, *current;
int len_er, count_trick_1, count_new, *inherit_adj;
...
//    for (int l = index_first[threadid]; l < index_first[threadid + 1]; ++l)
    ...
    for (int i = index_first[threadid]; i < index_first[threadid + 1]; ++i) {
        for (int j = 0; j < count_neg; ++j) {
            ...
            if (inherit_adj[sign[0][i]] == sign[2][j] || inherit_adj[sign[2][j]] == sign[0][i]) {
                pthread_mutex_lock(&lock_x);
                count_new++;
                current = (struct er_new_info *) malloc(sizeof(struct er_new_info));
                current->pos = i;
                current->neg = sign[2][j];
                current->pos_v = value[sign[0][i]];
                ...
                if (birth_index[sign[0][i]] < birth_index[sign[2][j]])
                    max_birth = birth_index[sign[2][j]];
                else
                    max_birth = birth_index[sign[0][i]];
                flag = 0;
