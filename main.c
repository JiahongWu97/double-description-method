...
//    print_mat_2(n, n, er);
    ...
    unsigned long long count_pos, count_zero, count_neg, len_er = n, count_new, count_er_all, count_trick_1;
...
//        printf("time to build active point: %f\n", (clock() - time1) * 1.0 / 1000);//看起来没多少
//        print_mat_2_bool(len_er, ii, active_point);
        count_pos = 0;
        count_zero = 0;
        count_neg = 0;
        int value[len_er];
        ...
        for (int i = 0; i < count_pos; ++i) {
            for (int j = 0; j < count_neg; ++j) {
                ...
                if (count_ineq >= n - 2) {
                    count_trick_1++;
                    count_point = 0;
                    for (unsigned long long k = 0; k < len_er; ++k) {
                        flag = 1;
                        for (int l = 0; l < count_ineq; ++l) {
                            if (active_point[ineq_reg[l]][k] == 0) {
                                flag = 0;
                                break;
                            }
                        }
...
/*
        ...
        while (current != NULL) {
            free(current);
            current = current->next;//这样好像就是会出错
        }
*/
        free(active_ineq);
        free(er);
