...
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r_g_set.exe multi_mutex.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/lex_set.exe multi_mutex.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/lex_rank.exe multi_mutex.c -pthread
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r_g_rank.exe multi_mutex.c -pthread


int ineq[m][n], ineq_order[m], (*er)[n], ii;
_Bool (*active_ineq)[m];
struct er_new_info *head, *prev, *current;
unsigned long long len_er, count_trick_1, count_new;
...
void *Thread(void *id) {
    ...
    printf("%dth started.\n", threadid);
    unsigned long long count_pos, count_zero, count_neg, point_temp, jj;
    count_pos = 0;
    count_zero = 0;
    count_neg = 0;
    int value[len_er];
...
//    printf("%.1E %.1E %.1E \n", (double) count_pos, (double) count_zero, (double) count_neg);
    ...
    //对+切割
    unsigned long long segment = count_pos / NUM_THREADS;
    unsigned long long index_first[NUM_THREADS + 1];
...
//    for (int l = index_first[threadid]; l < index_first[threadid + 1]; ++l)
    ...
    int count_ineq, ineq_reg[m];
    double ineq_copy[m][n];
    _Bool flag;
    for (unsigned long long i = index_first[threadid]; i < index_first[threadid + 1]; ++i) {
...
//    print_mat_2(n, n, er);
    ...
    unsigned long long count_pos, count_zero, count_neg, count_er_all, point_temp, jj;
