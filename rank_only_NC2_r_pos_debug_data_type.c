...
//gcc -Wl,--stack,4294967296 -o ./cmake-build-debug/r.exe rank_only_NC2_r_pos.c -pthread
...
//        print_mat_2_long(3, len_er, sign);
        random_pos = (int *) malloc(count_pos * sizeof(int));
...
/*
        if (ii == 153) {
            print_mat_1(count_pos, sign[0]);
            print_mat_1(count_zero, sign[1]);
        }
*/
...
//            printf("pos binary: %d\n", inherit_adj[sign[0][i]]);
            ...
            if (father > -1) {
                ...
                inherit_adj_new[count_er_all] = -1;
            if (ii == 153) {
                printf("%d %d\n", father, inherit_adj_new[count_er_all]);
            }
            for (int j = 0; j < n; ++j) {
                er_new[count_er_all][j] = er[sign[0][i]][j];
            }
            if (ii == 153) {
                print_mat_1(n, er[sign[0][i]]);
                print_mat_1(n, er_new[count_er_all]);
                printf("finish er\n");
            }
            for (int j = 0; j < ii; ++j) {
                if (ii == 153) {
                    printf("ready to load active\n");
                    printf("%d %d\n", j, ii);
                    printf("old :%d\n", active_ineq[sign[0][i]][j]);
                    printf("judge :%d\n", active_ineq_new == NULL);
                    printf("new :%d", active_ineq_new[count_er_all][j]);
                }
                active_ineq_new[count_er_all][j] = active_ineq[sign[0][i]][j];
            }
            if (ii == 153) {
                print_mat_1(ii, active_ineq[sign[0][i]]);
                print_mat_1(ii, active_ineq_new[count_er_all]);
                printf("finish initial\n");
            }
            active_ineq_new[count_er_all++][ii] = 0;
            if (ii == 153) {
                print_mat_1(ii, active_ineq[sign[0][i]]);
                print_mat_1(ii + 1, active_ineq_new[count_er_all]);
                printf("finish active\n");
            }
        }
